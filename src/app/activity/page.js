    ...
    async function fetchActivities() {
      try {
        ...
        const res = await fetch(
          `https://ganesha-cms.vercel.app/api/activity?limit=100`
        );
        ...
        if (data.success) {
          ...
          const sortedData = data.data.sort((a, b) => {
            // Assuming 'date' is a property in your activity objects
            // Convert date strings to Date objects for accurate comparison
            const dateA = new Date(a.date);
            const dateB = new Date(b.date);

            // Sort in descending order (newest date first)
            // Subtracting dates gives a timestamp difference that 'sort' uses
            return dateB - dateA;
          });

          console.log("Sorted Data:", sortedData);

          // Use the sorted data to update your state
