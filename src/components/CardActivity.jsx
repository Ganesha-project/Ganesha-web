...
export const CardActivity = ({ activities, items }) => {
    ...
    const handlePrevActivity = () => {
        if (selectedIndex === null) return;
        setSelectedIndex((prevIndex) => (prevIndex !== null && prevIndex > 0 ? prevIndex - 1 : activities.length - 1));
        setImageIndex(0);
    };

    const handleNextActivity = () => {
        if (selectedIndex === null) return;
        setSelectedIndex((prevIndex) => (prevIndex !== null && prevIndex < activities.length - 1 ? prevIndex + 1 : 0));
        setImageIndex(0);
    };
                    ...
                    <div className="md:w-1/3 flex flex-col relative">
                        <div className="absolute w-full backdrop-blur-md flex p-4 items-center gap-2 border-b pb-3 border-neutral-500/50">
                            <div className="flex items-center justify-between w-full">
                                ...
                                {activity.ig && activity.instagramUrl && (
                                    <button
                                        onClick={(e) => handleInstagramClick(e, activity.instagramUrl)}
                        ...
                        <div className="flex flex-col justify-between h-full p-4 py-14 md:py-16 md:max-h-full max-h-[30lvh] overflow-x-scroll noBar">
                            <div>
                                ...
                                {!activity.longDesc ? (
                                    <p className="text-sm mt-2">{activity.descriptions}</p>
                                ) : (
                    ...
                    {displayedActivities.map((el, idx) => (
                        ...
                        >
                            ...
                            <div className="absolute inset-0 flex items-end p-1 md:p-3">
                                <h1 className={`${el.ig && "scale-0 group-hover:scale-100 translate-y-full group-hover:translate-y-0"} duration-300 text-white md:text-sm bg-black/25 px-2 py-1 rounded-lg backdrop-blur-lg truncate text-[10px]`}>
                                    {el.title}
                                </h1>
                            </div>
                        </div>
