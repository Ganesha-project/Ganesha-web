...
export const ReusableCards = ({ data, cardColor, typeColor, priceColor, buttonColor, label }) => {
    return (
        <>
            <section className="md:m-24 m-5 space-y-10">
                ...
                <div className="md:grid md:grid-cols-3 gap-5 flex flex-col justify-center">
                    {data.map((el, idx) => (
                        <div
                            key={idx}
                            className={`${cardColor ? cardColor : 'bg-cyan-100'} p-5 rounded-3xl space-y-5 relative hover:scale-[1.03] duration-300 ease-in-out hover:shadow-secondaryShadow hover:brightness-105`}
                        >
                            ...
                            <div>
                                {el.features.map((el, idx) => (
                                    <div key={idx} className="flex items-center gap-3">
                                        <span className="items-start">
                                            {el.status === true ? (<BsFillCheckCircleFill className="text-green-500" />) : (<BsFillXCircleFill className="text-red-500" />)}
                                        </span>
