...
export const ServicesMenu = ({ expandedId, onClose }) => {
  ...
  const filteredMain = useMemo(() => {
    return categorizedServices.main.filter(
      (cat) =>
    ...
    <>
      ...
      <section className={`${expandAnimationClass}`}>
        ...
        <div
          className={`${
            visible === true
              ...
              : "translate-y-[500%] opacity-0 scale-95"
          } duration-300 ease-in-out flex items-center justify-center gap-2 w-full`}
        >
          <button
            onClick={closeSearch}
            className="h-[40px] w-[40px] aspect-square rounded-full dark:bg-darkColor/50 bg-lightColor/50 text-[14px] flex items-center justify-center shadow-mainShadow"
          >
            <RxCross2 className="opacity-50" />
          </button>
          <label
            className={`origin-bottom duration-300 ease-in-out z-[60] input !max-h-[40px] input-md dark:bg-darkColor/50 bg-lightColor/50 focus-within:dark:bg-darkColor/50 focus-within:bg-lightColor/50  focus-within:ring-opacity-0 focus-within:outline-none rounded-full flex items-center w-full gap-[6px] shadow-mainShadow`}
          >
        ...
        <div
          ...
          } duration-300 ease-in-out grid grid-cols-8 gap-4`}
        >
          <div className="col-span-3">
            {visible === false ? (
              <button
                onClick={() => setVisible(true)}
                className="flex items-center gap-[2px] py-[1px] px-2 rounded-full bg-neutral-300/30 dark:bg-neutral-500/20 text-[12px] font-bold text-darkColor/90 tracking-normal dark:text-lightColor/90"
              >
                Cari Layanan
                <div className="">
              ...
              <button
                ...
                className="flex items-center gap-[2px] py-[1px] px-2 rounded-full bg-neutral-500/20 text-[10px] font-bold text-darkColor/90 tracking-normal dark:text-lightColor/90"
              >
                Tutup
                <div className="">
          ...
          <div className="col-span-3">
            <p
              className={`${expandAnimationClass} duration-500 text-darkColor/70 tracking-normal dark:text-lightColor/70 text-xs font-bold block  rounded-full mb-3 w-fit`}
            >
              Layanan Lainnya
            </p>
          </div>
      ...
      <section
        className={`${
          visible === false ? "-translate-y-8" : "translate-y-1"
        } duration-300 ease-in-out grid gap-4 grid-cols-8`}
      >
                ...
                {filteredMain.map((cat, index) => (
                  ...
                  >
                    ...
                    <div className="z-10 space-y-1">
                      <h1 className="text-[15px] font-bold brightness-50 dark:brightness-150 opacity-60 dark:opacity-70">
                        {cat.label}
                      </h1>
                      <p className="flex gap-1 flex-wrap">
                        {cat.subs.map((sub, subIndex) => (
                          <span
                            key={subIndex}
                            className="text-darkColor/50 dark:text-lightColor/50 text-[9px] block px-2 py-1 bg-darkColor/10 dark:bg-lightColor/10 rounded-full"
                          >
                            {sub}
                          </span>
                        ))}
            ...
            <div className="col-span-3">
              <div className="grid grid-cols-2 gap-4">
                {filteredOthers
                  .filter((cat) => cat.visibility)
                  .map((cat, index) => (
                    <Link onClick={onClose} href={cat.href} key={index}>
                      <div
                        key={index}
                        className={`${
                          cat.visibility === true ? "block" : "hidden"
                        } ${expandAnimationClass} grow parent transition-transform relative overflow-hidden w-full h-full dark:bg-darkColor/50 bg-lightColor/50 rounded-3xl shadow-mainShadow flex p-3 hover:bg-opacity-20 hover:scale-95`}
                      >
                        {/* Glow element */}
                        <div
                          className="absolute top-0 left-0 w-[50%] h-[50%] rounded-full blur-3xl opacity-80"
                        ...
                        <div className="flex flex-row items-center gap-2 text-sm w-fit h-fit z-10">
                          ...
                          <p className="font-bold text-xs brightness-50 dark:brightness-100 leading-tight truncate-last-2 opacity-60 dark:opacity-70">
                            {cat.label}
                          </p>
                        </div>
                      </div>
