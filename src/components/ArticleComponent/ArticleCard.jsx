            ...
            <section className="space-y-5">
                ...
                <div className="md:grid md:grid-cols-4 flex flex-col gap-5">
                    {data?.data?.map((el, idx) => {
                        ...
                        const categoryData = el?.attributes?.category?.data?.attributes;
                        const categoryName = categoryData?.ArticleCategory || "Uncategorized";
                        const categorySlug = slugify(categoryName);
                        
                        return (
                            <Link
                                key={idx}
                                href={'/article/' + categorySlug + "/" + (el.attributes.Slug || slugify(el.attributes.Title))}
