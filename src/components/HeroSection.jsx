  ...
  return (
    <section className="hidden relative z-0 min-h-screen h-[130lvh] md:h-[185lvh] 2xl:h-[165lvh] md:flex flex-col items-center bg-lightColor/70 dark:bg-darkColor/50 overflow-hidden">
      ...
      <div className="relative z-20 flex flex-col items-center text-center mx-5 mt-18 md:mt-40">
        ...
        <div>
          <span
            className={`absolute mx-auto py-4 flex border w-fit blur-[20px] opacity-50 ${TextMainGradient} text-3xl md:text-6xl box-content font-extrabold text-center select-none`}
          >
            Transformasi Bisnis Anda <br /> Bersama Ganesha Consulting
          </span>
          <h1
            className={`relative py-4 justify-center flex items-center ${TextMainGradient} text-3xl md:text-6xl font-extrabold text-center`}
          >
            Transformasi Bisnis Anda <br /> Bersama Ganesha Consulting
          </h1>
        </div>
        <p className="text-sm md:text-lg mb-6 max-w-3xl opacity-90">
          Temukan solusi lengkap untuk semua kebutuhan bisnis Anda di Ganesha
          Consulting! Kami memberikan layanan all-in-one untuk memenuhi segala
          kebutuhan Anda!
        </p>
        <Link href={"https://api.whatsapp.com/send?phone=628887127000"} target="_blank" >
      ...
      {mounted && (
        <div className="block md:hidden relative z-20 mt-16 md:w-full transition-transform duration-500 ease-out rounded-3xl md:overflow-hidden pointer-events-none md:my-0 !my-auto">
          <ClientPhotos
            titleVisibility="hidden"
            padding="!p-0"
            padding2="!p-10"
      ...
      {/* Govern Logo Section */}
      <p className="text-sm text-center z-20">Resmi terdaftar di:</p>
